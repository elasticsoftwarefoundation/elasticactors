...
public final class TopicHelper {
    ...
    private static final String DEFAULT_MIN_IN_SYNC_REPLICAS = "2";

    private TopicHelper() {

    }

    public static void ensureTopicsExists(String bootstrapServers,
                                          String nodeId,
                                          int nodePartitions,
                                          InternalActorSystem internalActorSystem) throws Exception {
        //"bootstrap.servers"
        ...
        // support for clusters with less than 3 brokers (mainly used for testing)
        DescribeClusterResult clusterDefinition = adminClient.describeCluster();
        int brokers = clusterDefinition.nodes().get().size();
        short replicationFactor = brokers >= 3 ? DEFAULT_REPLICATION_FACTOR : new Short((short) brokers);
        String mininSyncReplicas = brokers >= 2 ? DEFAULT_MIN_IN_SYNC_REPLICAS : ""+brokers;
        ...
        if(!availableTopics.containsKey(messagesTopicName)) {
            ...
            NewTopic topic = new NewTopic(messagesTopicName, internalActorSystem.getNumberOfShards(), replicationFactor);
            ...
            configs.put(MIN_IN_SYNC_REPLICAS_CONFIG, mininSyncReplicas); // QUORUM writes
        ...
        if(!availableTopics.containsKey(nodeMessagesTopicName)) {
            NewTopic topic = new NewTopic(nodeMessagesTopicName, nodePartitions, replicationFactor);
            Map<String, String> configs = new HashMap<>();
            configs.put(CLEANUP_POLICY_CONFIG, CLEANUP_POLICY_DELETE);
            configs.put(MAX_MESSAGE_BYTES_CONFIG, "10485760");  // 1MB max message size
            configs.put(MIN_IN_SYNC_REPLICAS_CONFIG, mininSyncReplicas); // QUORUM writes
        ...
        if(!availableTopics.containsKey(persistentActorsTopicName)) {
            NewTopic topic = new NewTopic(persistentActorsTopicName, internalActorSystem.getNumberOfShards(), replicationFactor);
            Map<String, String> configs = new HashMap<>();
            ...
            configs.put(MIN_CLEANABLE_DIRTY_RATIO_CONFIG, "0.1"); // waste no more than 10%
            configs.put(MIN_IN_SYNC_REPLICAS_CONFIG, mininSyncReplicas); // QUORUM writes
            configs.put(RETENTION_MS_CONFIG, "-1"); // forever log retention for the messages
            configs.put(SEGMENT_MS_CONFIG, "604800000"); // roll over after 7 days even if not full
            topic.configs(configs);
            CreateTopicsResult result = adminClient.createTopics(Lists.newArrayList(topic));
            result.all().get(); // @todo: see if we didn't get an exception
        ...
        if(!availableTopics.containsKey(scheduledMessagesTopicName)) {
            NewTopic topic = new NewTopic(scheduledMessagesTopicName, internalActorSystem.getNumberOfShards(), replicationFactor);
            Map<String, String> configs = new HashMap<>();
            configs.put(CLEANUP_POLICY_CONFIG, CLEANUP_POLICY_COMPACT);
            configs.put(DELETE_RETENTION_MS_CONFIG, "86400000");  // 1 day tombstone retention
            configs.put(MAX_MESSAGE_BYTES_CONFIG, "20971520");  // 2MB max message size
        ...
        if(!availableTopics.containsKey(actorSystemEventListenersTopicName)) {
            NewTopic topic = new NewTopic(actorSystemEventListenersTopicName, internalActorSystem.getNumberOfShards(), replicationFactor);
            Map<String, String> configs = new HashMap<>();
            configs.put(CLEANUP_POLICY_CONFIG, CLEANUP_POLICY_COMPACT);
            configs.put(DELETE_RETENTION_MS_CONFIG, "86400000");  // 1 day tombstone retention
            configs.put(MAX_MESSAGE_BYTES_CONFIG, "20971520");  // 2MB max message size
